...
def spotify_callback(request: Request):
    ...
    if response.status_code != 200:
        return JSONResponse({"error": "Failed to exchange code for token", "details": response.text})
    ...
    if not access_token:
        return JSONResponse({"error": "Access token missing in response"})
    ...
    if user_response.status_code != 200:
        return JSONResponse({"error": "Failed to fetch user info", "details": user_response.text})
    ...
    if not spotify_user_id:
        return JSONResponse({"error": "Spotify user ID missing in user info"})
